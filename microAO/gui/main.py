...
from microAO.aoMetrics import metric_function, metric_names
...
class MicroscopeAOCompositeDevicePanel(wx.Panel):
    def __init__(self, parent, device):
        ...
        # Button to set sensorless routine
    ...
    def OnSetRoutine(self, event: wx.CommandEvent) -> None:
        del event

        # Create routine lookup table and choices for choice dialog
        routine_lookup = {val.name(): key for key, val in routines.items()}
        routine_choices = list(routine_lookup.keys())

        # Create dialog and set routine if OK pressed
        dlg = wx.SingleChoiceDialog(
            self, "Select routine", "Routine", routine_choices,
            wx.CHOICEDLG_STYLE
            )
        if dlg.ShowModal() == wx.ID_OK:
            routine = routine_lookup[dlg.GetStringSelection()]
            self._device.set_sensorless_routine(routine)
